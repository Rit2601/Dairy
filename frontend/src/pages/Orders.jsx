    ...
    <motion.div {...pageTransition} className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <Link to="/categories" className="inline-block mt-4 btn-primary">Start Shopping</Link>
        </div>
      ) : (
        <div className="space-y-3">
          {orders.map((order, i) => {
            ...
            return (
              ...
              >
                <Link to={`/orders/${order._id}`} className="block bg-white rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-1">{order.items.length} item{order.items.length > 1 ? 's' : ''} • ₹{order.total}</p>
                    </div>
                  ...
                  <div className="flex gap-2 mt-3">
                    {order.items.slice(0, 3).map((item, j) => (
                      <img
                        key={j}
                        src={item.image || 'https://images.unsplash.com/photo-1550583724-b2692b85b150?w=60&fit=crop'}
                        alt={item.name}
